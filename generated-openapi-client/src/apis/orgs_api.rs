...
/// struct for passing parameters to the method [`leave_org`]
#[derive(Clone, Debug)]
...
/// struct for typed successes of method [`leave_org`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum LeaveOrgSuccess {
    Status204(),
    UnknownValue(serde_json::Value),
}
...
/// struct for typed errors of method [`leave_org`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum LeaveOrgError {
...
pub async fn leave_org(configuration: &configuration::Configuration, params: LeaveOrgParams) -> Result<ResponseContent<LeaveOrgSuccess>, Error<LeaveOrgError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<LeaveOrgSuccess> = serde_json::from_str(&local_var_content).ok();
        let local_var_result = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Ok(local_var_result)
    } else {
        let local_var_entity: Option<LeaveOrgError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
